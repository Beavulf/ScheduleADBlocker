...
export default async function enableOrDisableUser(
  ...
  logger.info(
    `Изменение статуса пользователя ${isRecall ? '(Отзыв) ' : ''}(${status === '512' ? 'ВКЛ.' : 'ВЫКЛ.'}): ${login} по приказу ${task.order ?? '-разовая'} ${new Date().toLocaleString()}`,
    { label: 'cron' },
  );
  ...
  if (!foundUsers.length) {
    logger.error(
      `Пользователь не найден в AD ${isRecall ? '(Отзыв)' : ''}: ${login}`,
      { label: 'cron' },
    );
    // записываем в файл людей, которые не обработались
    await NotFoundUserToFile(
  ...
  if (foundUsers.length > 1) {
    logger.error(
      `Найдено несколько пользователей с логином ${isRecall ? '(Отзыв)' : ''}: ${login}`,
      { label: 'cron' },
    );
    // записываем в файл людей, которые не обработались
    await NotFoundUserToFile(
  ...
  if (!distinguishedName) {
    logger.error(
      `Отсутствует distinguishedName для пользователя ${isRecall ? '(Отзыв)' : ''}: ${login} - пользователь не будет изменён.`,
      { label: 'cron' },
    );
    // записываем в файл людей, которые не обработались
    await NotFoundUserToFile(
  ...
  try {
    ...
    await ldapService.enableOrDisableUserLdapts(status, {
      userDn: distinguishedName,
    });
    logger.info(
      `Пользователь успешно ${status === '512' ? 'ВКЛЮЧЕН' : 'ВЫКЛЮЧЕН'} ${isRecall ? '(Отзыв)' : ''}: ${login} - ${distinguishedName} ${new Date().toLocaleString()}`,
      { label: 'cron' },
    );
    return true;
  } catch (err) {
