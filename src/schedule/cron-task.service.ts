...
export class ScheduleCronTaskService {
     ...
     */
    private async runTask(taskName: string, taskFn: () => Promise<void>, timeoutMs = 20000): Promise<void> {
        const startedAt = Date.now();
        try {
            await Promise.race([
                taskFn(),
                // Если задача не завершилась за timeoutMs — выбрасываем ошибку таймаута
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Задача ${taskName} не выполнена за ${timeoutMs}мс`)), timeoutMs)),
            ]);
            this.logger.info(`Задача ${taskName} выполнена за ${Date.now() - startedAt}мс`, { label: 'cron', task: taskName });
        } catch (error) {
            this.logger.error(
    ...
    async handleScheduledTasks() {
        ...
        if (this.isRunning) {
            ...
            const now = Date.now();
            if (now - this.lastSkipWarnAt > 60_000) { // не чаще раза в минуту
                this.logger.warn(`Предыдущий запуск ещё выполняется — пропущено тиков: ${this.skippedTicksCount}`, { label: 'cron', skipped: this.skippedTicksCount });
        ...
        try {
            ...
            if (this.skippedTicksCount > 0) {
                this.logger.info(`До запуска было пропущено тиков: ${this.skippedTicksCount}`, { label: 'cron', skipped: this.skippedTicksCount });
                this.skippedTicksCount = 0;
            }

            await this.runTask('task-to-start-recall', () => handleTaskToStartRecall.call(this));
            await this.runTask('task-to-end-recall', () => handleTaskToEndRecall.call(this));
            await this.runTask('task-to-start', () => handleTaskToStart.call(this));
            await this.runTask('task-to-end', () => handleTaskToEnd.call(this));
            await this.runTask('task-to-onetime', () => handleTaskToOneTime.call(this));
            this.logger.info(`Все подзадачи завершены за ${Date.now() - start}мс`, { label: 'cron' });
        } finally {
            this.isRunning = false;
            this.logger.info(`Задача по обработке расписания блокировок завершена (${Date.now() - start}мс).`, { label: 'cron' });
        }
        console.log(`${job.isActive} - END - ${job.nextDate().toISODate()} - ${job.cronTime.sendAt()}`);
    ...
    async fireOnTick(taskName: string):Promise<Boolean>{
        ...
        try{
            ...
            this.logger.error(`Ошибка при попытке горячего выполнения в задаче ${taskName}: ${(err as Error)?.message}`, { label: 'cron' });
            throw new InternalServerErrorException(`Ошибка при попытке горячего запуска задачи ${taskName}: ${(err as Error)?.message}`);
        }
        return true;
    }
    ...
    async stopTask(taskName:string):Promise<Boolean>{
        ...
        try {
            ...
            this.logger.error(`Ошибка при попытке остановки задачи ${taskName}: ${(err as Error)?.message}`, { label: 'cron'})
            throw new InternalServerErrorException(`Ошибка при попытке остановки задачи ${taskName}: ${(err as Error)?.message}`)
        }
        return true;
    }
    ...
    async startTask(taskName:string):Promise<Boolean>{
        ...
        try {
            ...
            this.logger.error(`Ошибка при попытке запуска задачи ${taskName}: ${(err as Error)?.message}`, { label: 'cron'})
            throw new InternalServerErrorException(`Ошибка при попытке запуска задачи ${taskName}: ${(err as Error)?.message}`)
        }
        return true;
    }
