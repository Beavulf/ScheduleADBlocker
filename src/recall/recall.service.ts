...
export class RecallService {
  ...
  async checkDuplicateRecall(idSchedule: string, order: string) {
    try {
      ...
      throw new InternalServerErrorException(
        `Ошибка при работе с БД - ${err.message}`,
      );
    }
  }
  ...
  async create(
    ...
    const findedRecall = await this.checkDuplicateRecall(
      idSchedule,
      input.order,
    );
    ...
    if (findedRecall) {
      this.logger.error(
        `Отзыв на эту запись уже существует: (${input.order}, ${idSchedule}) пользователем ${authUsername}`,
      );
    ...
    this.logger.info(
      `Запись отзыва создана: ID ${idSchedule} - ${createdRecall.order} пользователем ${authUsername}`,
    );
    return true;
  }
  ...
  async update(
    ...
    this.logger.info(
      `Запись об отзыве изменена - ${updatedRecall.order} пользователем ${authUsername}`,
    );
    return true;
  }
  ...
  async getArchiveRecalls(options?: {
    ...
    const { filter, sort, skip, take } = options || {};
    const recalls: RecallGetModel[] =
      await this.prismaService.archiveRecall.findMany({
        where: filter,
        orderBy: sort ? { [sort.field]: sort.order || 'asc' } : undefined,
        skip,
        take,
