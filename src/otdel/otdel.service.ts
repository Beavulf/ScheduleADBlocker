...
export class OtdelService {
  ...
  async create(input: CreateInput, authUsername: string): Promise<boolean> {
    ...
    this.logger.info(
      `Отдел создан: ${input.name} пользователем ${authUsername}`,
    );
    return true;
  }
  ...
  async update(
    ...
    this.logger.info(
      `Отдел изменен: ${input.name} пользователем ${authUsername}`,
    );
    return true;
  }
  ...
  async delete(id: string, authUsername: string): Promise<boolean> {
    ...
    this.logger.info(
      `Отдел удален: ${findedOtdel.name} пользователем ${authUsername}`,
    );
    return true;
  }
