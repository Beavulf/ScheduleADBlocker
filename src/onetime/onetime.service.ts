...
export class OnetimeService {
  ...
  async checkDuplicate(login: string, date: Date) {
    ...
    const duplicateOneTime: OneTime | null =
      await this.prismaService.oneTime.findFirst({
        where: {
          login,
          date: {
            gte: dayStart,
  ...
  async findById(id: string): Promise<OneTime> {
    const findedOneTime: OneTime =
      await this.prismaService.oneTime.findUniqueOrThrow({
        where: {
          id,
        },
      });
  ...
  async create(
    ...
    const findedOneTime: OneTime | null = await this.checkDuplicate(
      input.login,
      input.date,
    );
    ...
    if (findedOneTime) {
      this.logger.error(
        `Запись разовой задачи уже существует: ${input.login} - ${input.date} пользователем ${authUsername})`,
      );
    ...
    this.logger.info(
      `Запись разовой задачи создана: ${createdOneTime.fio} пользователем ${authUsername}`,
    );
  ...
  async delete(id: string, authUsername: string): Promise<boolean> {
    ...
    this.logger.info(
      `Запись расписания удалена: ${deletedOneTime.fio}-${deletedOneTime.date} пользователем ${authUsername}`,
    );
  ...
  async update(
    ...
    this.logger.info(
      `Запись разовой задачи изменена ${updatedOneTime.fio} пользователем ${authUsername}`,
    );
  ...
  async getOneTimes(options?: {
    ...
    const oneTimes: OneTimeGetModel[] =
      await this.prismaService.oneTime.findMany({
        where: filter,
  ...
  async toArchive(id: string): Promise<boolean> {
    try {
      ...
      const isCompleate = await this.prismaService.$transaction(async (tx) => {
        ...
        if (!oneTime) {
          this.logger.warn(
            `Разовая задача при попытке архивирования не найдена: ${id}`,
            { label: 'cron' },
          );
          return false;
        }
      ...
      this.logger.info(
        `Архивирование и обновление выполнено (Разовая): ${id}`,
        { label: 'cron' },
      );
  ...
  async getArchiveOneTimes(options?: {
    ...
    const { filter, sort, skip, take } = options || {};
    const oneTimes: OneTimeGetModel[] =
      await this.prismaService.archiveOneTime.findMany({
        where: filter,
        orderBy: sort ? { [sort.field]: sort.order || 'asc' } : undefined,
        skip,
        take,
