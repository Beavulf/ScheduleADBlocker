...
export class LdapService {
  ...
  constructor(
    ...
    this.LDAP_ADMIN_PASSWORD = this.configService.getOrThrow(
      'LDAP_ADMIN_PASSWORD',
    );
  }
  ...
  getOptsByCn(value: string): ldap.SearchOptions {
    const orFilter = new ldap.OrFilter({
      filters: [
    ...
    const finalFilter = new ldap.AndFilter({
      filters: [
  ...
  async createAndBindLdapClient(): Promise<ldap.Client> {
    ...
    await new Promise((resolve, reject) => {
      client.bind(this.LDAP_ADMIN_LOGIN, this.LDAP_ADMIN_PASSWORD, (err) => {
        if (err) {
          this.logger.error(`Ошибка при подключении к АД - ${err.message}.`);
          reject(
            new ConflictException(
              `Ошибка при подключении к АД: ${err.message}`,
            ),
          );
          return;
        }
      ...
      client.search(
        ...
        (err, res) => {
          if (err) {
            ...
            client.unbind();
            this.logger.info(
              `Ошибка при поиске пользователя - ${cnOrSamaccountname} (${err.message})`,
            );
            reject(
              new ConflictException(
                `Ошибка при поиске пользователя - ${err.message}`,
              ),
            );
            return;
          }
          ...
          res.on('searchEntry', (entry) => {
            ...
            const user = attributes.reduce((acc, field) => {
              if (field === 'memberOf') {
                acc[field] = entry.attributes.find(
          ...
          res.on('end', (result) => {
            client.unbind();
            this.logger.info(
              `Поиск окончен (${result?.status}) - закрытие соединения. Возвращено записей ${users.length}.`,
            );
            resolve(users);
          });
        },
  ...
  // ******************************ФУНКЦИИ С БИБЛИОТЕКОЙ LDAPTS**********************************

  // создание клиента ldapts под данными "админа"
  /**
   * Создаёт и возвращает LDAP-клиент ldapts, предварительно выполняя bind под учётными данными администратора.
   * Логирует попытку подключения и результат. В случае ошибки выбрасывает исключение.
   * @returns {Promise<Client>} Промис с авторизованным LDAP-клиентом ldapts
   * @throws {ConflictException} В случае ошибки подключения или авторизации
   */
  async createAndBindLdaptsClient(): Promise<Client> {
    // Создаём новый экземпляр LDAP-клиента ldapts с заданными таймаутами
    ...
    this.logger.info('Попытка соединения к АД через ldapts.');

    try {
      // Выполняем bind (аутентификацию) под учётными данными администратора
      await client.bind(this.LDAP_ADMIN_LOGIN, this.LDAP_ADMIN_PASSWORD);
      this.logger.info('Подключение к АД через ldapts успешно выполнено.');
      return client;
    } catch (err: any) {
      this.logger.error(
        `Ошибка при подключении к АД через ldapts - ${err.message}.`,
      );
  ...
  // получение Change объекта для модифицирования пользователя через ldapts (отключение-514/включение-512)
  /**
   * Создаёт объект Change для модификации атрибута userAccountControl через ldapts.
   * @param state - новое значение userAccountControl (например, '512' или '514')
   * @returns Change объект для ldapts
   */
  private getChangeUserAccountControlLdapts(state: string): Change {
  ...
  // изменения записи пользователя через ldapts по distinguishedName
  /**
   * Модифицирует атрибут userAccountControl для пользователя в LDAP через библиотеку ldapts.
   * Используется для включения (512) или отключения (514) пользователя.
   * Поддерживает DN с кириллицей через нормализацию.
   * @param client - экземпляр LDAP клиента ldapts с активным соединением
   * @param state - новое значение userAccountControl (например, '512' или '514')
   * @param userDn - объект с DN пользователя (ModifyInput)
   * @returns Promise<void> - успешно завершится при удачной модификации, либо выбросит исключение при ошибке
   */
  private async modifyUserLdapts(
    client: Client,
    state: string,
    userDn: ModifyInput,
  ): Promise<void> {
    // Нормализуем DN для корректной работы с кириллицей
    const normalizedDn = userDn.userDn?.normalize('NFC') || userDn.userDn;

    this.logger.info(
      `Попытка изменения статуса пользователя через ldapts - ${normalizedDn}.`,
    );

    try {
      await client.modify(
        normalizedDn,
        this.getChangeUserAccountControlLdapts(state),
      );
      // Логируем успешное изменение статуса пользователя
      this.logger.info(
        `Пользователь ${normalizedDn} успешно изменен на статус ${state} через ldapts.`,
      );
    } catch (err: any) {
      // Логируем и пробрасываем ошибку, если изменение не удалось
      this.logger.error(
        `Ошибка при изменении пользователя через ldapts - ${err.message}. DN: ${normalizedDn}`,
      );
      throw new ConflictException(
        `Ошибка при изменении пользователя через ldapts - ${err.message}.`,
      );
    }
  }

  // включение или отключение пользователя через ldapts 514 off 512 on
  /**
   * Включает или отключает пользователя в Active Directory через библиотеку ldapts.
   * Использует модификацию атрибута userAccountControl.
   * @param state - новое значение userAccountControl ('512' для включения, '514' для отключения)
   * @param userDn - объект с DN пользователя (ModifyInput)
   * @returns Promise<boolean> - возвращает true при успешной модификации
   * @throws {ConflictException} В случае ошибки модификации
   */
  async enableOrDisableUserLdapts(
    state: string,
    userDn: ModifyInput,
  ): Promise<boolean> {
    const client = await this.createAndBindLdaptsClient();
    try {
      await this.modifyUserLdapts(client, state, userDn);
      return true;
    } finally {
      await client.unbind();
    }
